local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/heap-v/Public-Scripts/main/MEEPSHITTERlibrary.lua"))()
...
local function checkdir()
    if not isfolder("MEEPSHITTER") then
        makefolder("MEEPSHITTER")
    end
end

local guiname = "MEEPSHITTER"
...
if isfile("MEEPSHITTER\\clr.txt") then
    local R,G,B = unpack(readfile("MEEPSHITTER\\clr.txt"):split(";"))
...
Welcome:AddLabel("Thank you for using MEEPSHITTER.")
Welcome:AddButton("Join Our Discord Server",function()
    local Settings = {
        InviteCode = "xBCxAMpdF6" --add your invite code here (without the "https://discord.gg/" part)
    }
...
local fgs = Local:AddSwitch("Free Gamepasses",function(b)
	checkdir()
	writefile("MEEPSHITTER\\freegamepasses.txt",tostring(b))
	game.Players.LocalPlayer:SetAttribute("PLUS",b)
	game.Players.LocalPlayer:SetAttribute("BoomBox",b)
end)

if isfile("MEEPSHITTER\\freegamepasses.txt") and readfile("MEEPSHITTER\\freegamepasses.txt") == "true" then
	fgs:Set(true)
end
...
Local:AddTextBox("Set World Background Music ID",function(id)
    if tonumber(id) then
        checkdir()
        writefile("meepshitter\\bgmusic.txt",id)
...
if isfile("MEEPSHITTER\\bgmusic.txt") then
    require(game:GetService("ReplicatedStorage"):WaitForChild("ClientClasses"):WaitForChild("VirtualWorld")).SetBackgroundMusic(readfile("MEEPSHITTER\\bgmusic.txt"),.5,true)
end
...
hori:AddButton("Fuck",function()
	NBox:Set(game:HttpGet("https://raw.githubusercontent.com/heap-v/Public-Scripts/main/g.lua"))
end)
...
local dclrp = Settings:AddColorPicker(function(clrp)
	checkdir()
	writefile("MEEPSHITTER\\clr.txt",table.concat({clrp.R,clrp.G,clrp.B},";"))
end)
dclrp:Set(clr)
